...
func (txn *MvccTxn) GetValue(key []byte) ([]byte, error) {
	// Your Code Here (4A).
	wIter := txn.Reader.IterCF(engine_util.CfWrite)
	defer wIter.Close()
	for wIter.Seek(EncodeKey(key, txn.StartTS)); wIter.Valid(); wIter.Next() {
		wItem := wIter.Item()
		userKey := DecodeUserKey(wItem.Key())
		if !bytes.Equal(userKey, key) {
			return nil, nil
		}

		wValue, err := wItem.Value()
		if err != nil {
			return nil, err
		}
...
func (txn *MvccTxn) MostRecentWrite(key []byte) (*Write, uint64, error) {
	...
	return write, decodeTimestamp(wItem.Key()), nil
}
